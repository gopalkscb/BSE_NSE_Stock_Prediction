...
9. Testing scope for MVP1: basic smoke tests only (1-3 per module, pytest). Property-based tests (hypothesis), Playwright E2E tests, full integration test suites, and frontend unit tests (Vitest + RTL) are deferred to the test hardening phase (tracked in MVP3 scope as MVP1b).

---

### Requirement 9 — Basic Observability

**User Story:** As an operator, I want basic health checking and structured request logging so I can monitor the application and diagnose issues.
...
### Requirement 10 — Observability UI & FAQ Library

**User Story:** As an operator, I want a dedicated observability dashboard showing live metrics, error logs, per-ticker health signals, and a searchable FAQ/debug knowledge base, so that I can monitor application health, diagnose issues, and understand metric thresholds.

#### Acceptance Criteria

1. THE API Server SHALL store observability data (metrics, errors, ticker health) in a SQLite database at `data/observability.db` using `aiosqlite`. The database SHALL survive server restarts.
2. THE API Server SHALL expose `GET /api/v1/observability/metrics` returning aggregated counters (total_requests, avg_latency_ms, cache_hits, cache_misses, total_errors, total_warnings, failed_ticker_rate) and the 50 most recent metric events.
3. THE API Server SHALL expose `GET /api/v1/observability/errors` accepting query parameters `limit` (default 50), `offset` (default 0), and `level` (optional: "ERROR" or "WARNING") for paginated, filterable error log retrieval.
